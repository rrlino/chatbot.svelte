<script lang="ts">
	...
	import { Plus, Download, RefreshCw, Search, SlidersHorizontal, X, ClipboardList, Users, AlertTriangle, AlertCircle, Clock, BarChart3, Pencil, Trash2, ToggleRight, ToggleLeft } from 'lucide-svelte';
	...
	let globalCategory = $state('');
	let globalRisk = $state('');
	...
	function filteredQuestions(): ParqQuestion[] {
		return questions.filter((q) => {
			if (globalSearch && !q.question_text.toLowerCase().includes(globalSearch.toLowerCase())) return false;
			if (globalCategory && q.category !== globalCategory) return false;
			if (globalRisk && q.risk_level !== globalRisk) return false;
			if (globalStatus === 'enabled' && !q.is_enabled) return false;
			if (globalStatus === 'disabled' && q.is_enabled) return false;
			return true;
		});
	}
	...
	function filteredAnswers(): ParqAnswer[] {
		return answers.filter((a) => {
			if (globalSearch && !a.user_name.toLowerCase().includes(globalSearch.toLowerCase()) && !a.question_text.toLowerCase().includes(globalSearch.toLowerCase())) return false;
			if (globalRisk && a.risk_level !== globalRisk) return false;
			return true;
		});
	}
...
<svelte:head>
	<title>PAR-Q Management (Alt) - TrueLocal AI</title>
</svelte:head>
...
<div class="p-6">
	...
	<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
		<div class="flex items-center justify-between mb-4">
			<h1 class="text-xl font-bold text-gray-900">PAR-Q Management</h1>
			<div class="flex items-center gap-1 text-gray-500 text-sm">
				<ClipboardList class="h-4 w-4" />
				<span class="font-medium">{questions.length} questions</span>
			</div>
		</div>

		<div class="flex flex-wrap items-end gap-3">
			<div class="flex-1 min-w-[200px]">
				<p class="block text-sm font-medium text-gray-700 mb-1">Search</p>
				<div class="relative">
					<Search class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
					<input type="text" bind:value={globalSearch} placeholder="Questions, users, or content" class="w-full pl-9 pr-8 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
					{#if globalSearch}
						<button onclick={() => (globalSearch = '')} class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"><X class="h-4 w-4" /></button>
					{/if}
				</div>
			</div>
			<div>
				<p class="block text-sm font-medium text-gray-700 mb-1">Category</p>
				<select bind:value={globalCategory} class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
			...
			<div>
				...
				<select bind:value={globalRisk} class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
			...
			<div>
				...
				<select bind:value={globalStatus} class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
			...
			<div class="flex items-center gap-1 pb-0.5">
				<button onclick={loadData} title="Apply" class="p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"><SlidersHorizontal class="h-4 w-4" /></button>
				<button onclick={clearGlobalFilters} title="Clear" class="p-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"><X class="h-4 w-4" /></button>
				<button onclick={loadData} title="Refresh" class="p-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"><RefreshCw class="h-4 w-4" /></button>
				<button onclick={exportData} title="Export" class="p-2 border border-green-300 text-green-700 rounded-lg hover:bg-green-50 transition-colors"><Download class="h-4 w-4" /></button>
				<button onclick={openCreateModal} title="New Question" class="p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"><Plus class="h-4 w-4" /></button>
			</div>
		</div>
	</div>
	...
	<div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
		<div class="flex border-b border-gray-200">
			<button onclick={() => (activeTab = 'questions')} class="flex items-center gap-2 px-6 py-3 text-sm font-medium border-b-2 transition-colors {activeTab === 'questions' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}">
				<ClipboardList class="h-4 w-4" />
				Questions
				<span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-700">{filteredQuestions().length}</span>
			</button>
			<button onclick={() => (activeTab = 'answers')} class="flex items-center gap-2 px-6 py-3 text-sm font-medium border-b-2 transition-colors {activeTab === 'answers' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}">
				<Users class="h-4 w-4" />
				Responses
				<span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">{filteredAnswers().length}</span>
			</button>
						...
						<tbody class="divide-y divide-gray-200">
							{#each filteredQuestions() as q (q.id)}
								<tr class="hover:bg-gray-50 transition-colors">
									...
									<td class="px-4 py-3 text-sm text-gray-500 capitalize">{q.category}</td>
